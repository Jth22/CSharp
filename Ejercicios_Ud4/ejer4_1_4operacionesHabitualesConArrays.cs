 ...
 */
 ...
 class Ejer4_1_4operacionesHabitualesConArrays
 {
	 ...
	 public static void Ejercicio4_1_4_5()
	 {
		...
		int cantidad = 0, posicionAInsertar = 0, datoaBorrar = 0;
		string opcionUsuario, datoAInsertar = " ";
		...
		do
		{
			...
			case "2": 
				if ( cantidad < CAPACIDAD )
				{
					...
					if ( posicionAInsertar < cantidad && cantidad < CAPACIDAD )
					{
			...
			case "3": 
				...
				{ 
					...
					datoaBorrar = Convert.ToInt32(Console.ReadLine());
				}
				catch( FormatException e )
				{
					Console.WriteLine("Dato erróneo" + e.Message);
